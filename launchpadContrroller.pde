  ...
  for (int iy = 0; iy < 8; iy++) {
    for (int ix = 0; ix < 9; ix++) {
      ...
      if (ButtonSelector(ix*35+603, iy*35+400, 30, 30) && mousePressed) {
        ...
        if (ModeSel_ == 6) {
          LPBs_mode[ix][iy] = 10 + selID;
        }
        LPBs_stdCol[ix][iy] = RG(selColRLow, selColGLow);
        //println("selColRLow : "+str(selColRLow));
        //println("selColGLow : "+str(selColGLow));

        //println("selColRHigh : "+str(selColRHigh));
        //println("selColGHigh : "+str(selColGHigh));


        LPBs_highlightCol[ix][iy] = RG(selColRHigh, selColGHigh);
